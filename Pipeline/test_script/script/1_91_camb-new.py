...
class cambriconOp(dsl.ContainerOp):
    ...
    def __init__(self, start_time):
        super(cambriconOp, self).__init__(
            ...
            command=['sh', '-c'],
            arguments=[
                "cd /home/dl-plateform/mlu100-example-arm/classification/ && ./run.sh &&  date > /output.txt && echo '$0' >> /output",
                start_time],
            file_outputs={
...
def time_stat():
    ...
    # arm = armOp(start.output).add_node_selector_constraint('beta.kubernetes.io/arch','arm64').add_volume(
    #     k8s_client.V1Volume(name='arm1',
    #                         host_path=k8s_client.V1HostPathVolumeSource(path="/root"))).add_volume_mount(
    #     k8s_client.V1VolumeMount(mount_path='/root', name='arm1'))

    arm = armOp(start.output).add_node_selector_constraint('kubernetes.io/hostname',"10.0.1.126").add_volume(
        k8s_client.V1Volume(name='arm1',
                            host_path=k8s_client.V1HostPathVolumeSource(path="/root"))).add_volume_mount(
        k8s_client.V1VolumeMount(mount_path='/root', name='arm1'))
    ...
    camb.add_node_selector_constraint('kubernetes.io/hostname',"10.0.1.126")


    amd1 = amd1Op(amd.output).add_node_selector_constraint('beta.kubernetes.io/arch','amd64').add_volume(
        k8s_client.V1Volume(name='amd2',
                            host_path=k8s_client.V1HostPathVolumeSource(path="/root"))).add_volume_mount(
        k8s_client.V1VolumeMount(mount_path='/root', name='amd2'))

    # arm1 = arm1Op(arm.output).add_node_selector_constraint('beta.kubernetes.io/arch','arm64').add_volume(
    #     k8s_client.V1Volume(name='arm2',
    #                         host_path=k8s_client.V1HostPathVolumeSource(path="/root"))).add_volume_mount(
    #     k8s_client.V1VolumeMount(mount_path='/root', name='arm2'))

    arm1 = arm1Op(arm.output).add_node_selector_constraint('kubernetes.io/hostname',"10.0.1.126").add_volume(
        k8s_client.V1Volume(name='arm2',
                            host_path=k8s_client.V1HostPathVolumeSource(path="/root"))).add_volume_mount(
        k8s_client.V1VolumeMount(mount_path='/root', name='arm2'))

    # arm_camb = armOp_camb(camb.output).add_node_selector_constraint('beta.kubernetes.io/arch','arm64').add_volume(
    #     k8s_client.V1Volume(name='camb2',
    #                         host_path=k8s_client.V1HostPathVolumeSource(path="/root"))).add_volume_mount(
    #     k8s_client.V1VolumeMount(mount_path='/root', name='camb2'))

    arm_camb = armOp_camb(camb.output).add_node_selector_constraint('kubernetes.io/hostname',"10.0.1.126").add_volume(
        k8s_client.V1Volume(name='camb2',
                            host_path=k8s_client.V1HostPathVolumeSource(path="/root"))).add_volume_mount(
        k8s_client.V1VolumeMount(mount_path='/root', name='camb2'))
