...
class wgsmer(dsl.ContainerOp):
    ...
    def __init__(self, validate, validate1):
...
class wgssplit1(dsl.ContainerOp):
    ...
    def __init__(self, validate):
        super(wgssplit1, self).__init__(
            ...
            image='10.0.1.91:80/library/wgs-split-bam:latest',
            command=['./root/app/split.sh'],
            arguments=[
                '--validate', validate,
            ],
...
def wgs_demo():
    ...
    start = wgsstart().add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')
    # 实例化对象，挂载储存节点，指定计算节点，开始测序
    split = wgssplit(start.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')
    # 实例化对象，挂载储存节点，指定计算节点
    bwa = wgsbwa(split.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点
    bwa1 = wgsbwa1(split.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点
    sam = wgssam(bwa.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点
    sam1 = wgssam1(bwa1.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点
    mer = wgsmer(sam.output, sam1.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点
    sp1 = wgssplit1(mer.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点
    pic = wgspic(sp1.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点
    pic1 = wgspic1(sp1.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点
    ind = wgsind(pic.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')
    # 实例化对象，挂载储存节点，指定计算节点
    ind1 = wgsind1(pic1.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点
    bqs = wgsbqs(ind.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点
    bqs1 = wgsbqs1(ind1.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点
    cal = wgscal(bqs.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点
    cal1 = wgscal1(bqs1.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')

    # 实例化对象，挂载储存节点，指定计算节点，结束标志
    end = wgsend(cal.output, cal1.output).add_volume(k8s_client.V1Volume(name='wgs-data',host_path=k8s_client.V1HostPathVolumeSource(
        path="/mnt/xfs"))).add_volume_mount(k8s_client.V1VolumeMount(name='wgs-data', mount_path="/mnt/xfs")).add_node_selector_constraint(
        'beta.kubernetes.io/arch', 'amd64')
